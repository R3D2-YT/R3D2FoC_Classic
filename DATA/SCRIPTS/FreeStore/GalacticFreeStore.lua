-- $Id: //depot/Projects/StarWars/Run/Data/Scripts/FreeStore/GalacticFreeStore.lua#19 $
--/////////////////////////////////////////////////////////////////////////////////////////////////
--
...
--/////////////////////////////////////////////////////////////////////////////////////////////////
--
--              $File: //depot/Projects/StarWars/Run/Data/Scripts/FreeStore/GalacticFreeStore.lua $
--
...
function On_Unit_Service(object)
	...
	-- Don't shuffle the freestore during opening moves because it interrupts the custom staging
	if GlobalValue.Get(PlayerSpecificName(PlayerObject, "OPENING_MOVES_UNDERWAY")) == 1.0 then
		return
	end
...
function Find_Ground_Unit_Target(object)
	...
	if leader_planet and leader_planet.Get_Is_Planet_AI_Usable() and object.Can_Land_On_Planet(leader_planet) then
		if leader_planet == my_planet then
			if EvaluatePerception("Friendly_Land_Unit_Raw_Total", PlayerObject, leader_planet) < 1.5 * force_target then
				return leader_planet
			end	
		elseif EvaluatePerception("Friendly_Land_Unit_Raw_Total", PlayerObject, leader_planet) < force_target then
			return leader_planet
		end
	end
	...
	if priority_planet and priority_planet.Get_Is_Planet_AI_Usable() and object.Can_Land_On_Planet(priority_planet) then
		if priority_planet == my_planet then
			if EvaluatePerception("Friendly_Land_Unit_Raw_Total", PlayerObject, priority_planet) < 1.5 * force_target then
				return priority_planet
			end	
		elseif EvaluatePerception("Friendly_Land_Unit_Raw_Total", PlayerObject, priority_planet) < force_target then
			return priority_planet
		end
	end
	
	if my_planet and EvaluatePerception("Low_Ground_Defense_Score", PlayerObject, my_planet) > 0.5 and object.Can_Land_On_Planet(my_planet) then
		return nil
	end
